...
- отдельный выключатель между общей DC-шиной и ИБП не устанавливается: силовой тракт минимизируется ради снижения контактных потерь.
...
Выбор 200 А основан на входном токе ИБП 270 А: при симметричной работе двух батарей каждая ветвь несёт около 135 А, а MRBF 200 А работает примерно на 67,5 % номинала. При небольшой несимметрии 55/45 более нагруженная ветвь несёт около 148,5 А, что остаётся ниже 80 % номинала предохранителя. Кратковременная перегрузка ИБП 5400 Вт в течение 5 секунд также не должна вызывать ложного срабатывания исправного MRBF 200 А с подходящей времятоковой характеристикой.
