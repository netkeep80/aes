...
## Минимальный силовой тракт 12 В

Между общими DC-узлами и ИБП **не устанавливается отдельный автомат или рубильник**. После обязательных F1/F2 цепь выполняется максимально короткой:

```text
АКБ → F1/F2 → общие шины → одна короткая пара 35–50 мм² → ИБП
```

Причина — токи порядка сотен ампер при напряжении около 12 В. Дополнительный выключатель добавил бы внутренние контакты, два силовых наконечника и два болтовых соединения, увеличивая падение напряжения, нагрев и число потенциальных точек отказа. Для обслуживания система сначала полностью разгружается и обесточивается, после чего изоляция аккумуляторного банка выполняется снятием F1/F2 без тока.
